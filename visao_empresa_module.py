...
def clean_code(df1):
    ...
    return df1_cleaned
def order_metric(df_aux):
    fig = px.bar(df_aux, x='Order_Date', y='ID')
    return fig


def week_of_year(df1_cleaned):
...
def average_week_year_id(df1_cleaned):
...
def show_map(df1_cleaned):
    df_aux_grap = df1_cleaned.loc[:, ['City', 'Road_traffic_density', 'Delivery_location_latitude','Delivery_location_longitude']].groupby(
...
with tab2:
    with st.container():
        fig = week_of_year(df1_cleaned)
        st.plotly_chart(fig, use_container_width=True)

    with st.container():
        fig = average_week_year_id(df1_cleaned)
        st.plotly_chart(fig, use_container_width=True)

with tab3:
    # Exibe o mapa
    st.header('Geographical traffic locations')
    map = show_map(df1_cleaned)
    folium_static(map)
